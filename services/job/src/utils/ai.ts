...
export function simpleSkillScore(jobSkills: string[] = [], applicantSkills: string[] = []) {
...
export async function computeCompatibilityWithAI(jobText: string, applicantText: string) {
  ...
  try {
    ...
    const resp = await axios.post(
      url,
      { inputs: [jobText, applicantText], model: process.env.GEMINI_MODEL || "gemini" },
...
export async function computeCompatibility(jobSkills: string[] = [], applicantSkills: string[] = [], applicantText = "") {
